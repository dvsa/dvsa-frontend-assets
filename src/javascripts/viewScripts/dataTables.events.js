    ...
    var eListLs = $('#listLogs'),
        oTable = eListLs.dataTable({
            ...
            "fnServerData": function ( sSource, aoData, fnCallback ) {
                aoData.push({"name" :"goBack",          "value" :$('input[name="go_back_link"]').val()});
                ...
                $.getJSON( sSource, aoData, function (json) {
                    ...
                    $('table.dataTable tr td:nth-child(2n)').each(function(){
                        var tempDate = $(this).html(),
                            stringParts = tempDate.split(','),
                            newDate = stringParts[0];
                        $(this).html(newDate);

                    });

                    var eventsTitle = $('#events-title'),
                        eventTitleSource = $('#listLogs_info'),
                        eventsTitleText = $(eventTitleSource).html();

                    $(eventsTitle).html(eventsTitleText);
                    $(eventTitleSource).css('display', 'none');
;                });
            }
        });
    ...
    $(dateRangeFields).find('input[type=submit]').click(function(){
        isShowDateInput.val(1);
    });

    // Fix the error message bug when returning to the events list page form an individual event detail page
    // This is caused by the isShowDate var not being there, so it needs clearing when the search is submitted
    // eventsFilterForm
    var eventsForm = $('#eventsFilterForm');

    var clearDateSearchVisible = function() {

        var searchContents = $('#search').val();
        if(searchContents !== '' && searchContents.length > 2) {
            $(isShowDateInput).val('');
        }
    };

    $(eventsForm).on('submit', clearDateSearchVisible);
